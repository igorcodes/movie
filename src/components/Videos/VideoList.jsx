...
export default class VideoList extends React.Component {
	...
	getVideos = filters => {  //аргумент filters буду пробрасывать в эту функцию
		const {sort_by} = filters  //хочу sort_by брать с filters (аргумент с функции)
	...
	componentDidMount() { //этот МЖЦ срабатывает только 1 раз после первого рендера там где и нужно делать все наши ajax-запросы. 
		this.getVideos(this.props.filters);
																												/* 
	...
	componentDidUpdate(prevProps) {
		console.log("componentDidUpdate", prevProps); //особенность componentDidUpdate в том что тут уже все поменялось, рендер уже произошол, срабатывает после рендера, пропсы уже поменялись, имеем доступ к предыдущим пропсам 
